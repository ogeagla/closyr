...
- [ ] Webapp improvement: If the backend process is stopped in the middle of a job, right now we just clear the job from the Results section. can we actually just stop the job and then we can still see it + in the history? 
- [ ] Bug: Webapp / Job Results when job is stopped: it seems to show prior job's scoring method, scores and maybe other things. it needs to show the data for the done/stopped job.
